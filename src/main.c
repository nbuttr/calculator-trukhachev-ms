...
int main(int argc, char* argv[])
{
    ...
    // Пропускаем пробелы и получаем индекс первого значащего символа
    int i = 0;
    while (isspace(buffer[i])) {
        i++;
    }
    // Проверяем, что первый символ не является оператором
    if (buffer[i] == '-' || buffer[i] == '+' || buffer[i] == '*' || buffer[i] == '/' || buffer[i] == '%') {
        fprintf(stderr, "Error: Expression cannot start with operator '%c'\n", buffer[i]);
        return 1;
    }
